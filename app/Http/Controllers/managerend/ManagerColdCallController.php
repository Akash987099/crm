...
class ManagerColdCallController extends Controller
{
    public function ManagerColdCall()
    {
        return view('managerend.mcold-call');
    }
    public function Create_ManagerColdCall(Request $request)
    {
    ...
    public function View_Manager_coldcall(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($data)->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $btn = '<a class="btn btn-info btn-sm m-1 coldcall_id" href="' . route('manager.manager-coldcall-details', ['meetingid' => Crypt::encrypt($row->id)])  . '" title="View Data"> <i class="bi bi-eye"></i></a>';
                    if ($row->status == 1) {
                    ...
                    if (!DB::table('payment_details')->where('meeting_id', $row->id)->exists()) {
                        $btn .= '<a class="btn btn-primary btn-sm m-1" title="Update" href="' . route('manager.edit-mcold-call', ['id' => Crypt::encrypt($row->id)]) . '"> <i class="bi bi-pencil-square"></i></a>';
                        $btn .= '<a class="btn btn-info btn-sm m-1" title="Add Services" href="' . route('manager.add-coldcall-service', ['meetingid' => Crypt::encrypt($row->id)]) . '"> <i class="bi bi-plus-circle-fill"></i></a>';
                    }

                    $btn .= '<a class="btn btn-danger btn-sm m-1" title="Delete" href="' . route('manager.delete-coldcall', ['id' => Crypt::encrypt($row->id)]) . '"><i class="bi bi-trash"></i></a>';
                    return $btn;
                })
                ->rawColumns(['action'])
                ->make(true);
        }
        return view('managerend.mview-cold-call');
    }

    public function Edit_Manager_coldcall($id)
    {
        $edit_mcoldCall = Meeting::find(Crypt::decrypt($id));
        return view('managerend.edit-mcold-call', compact('edit_mcoldCall'));
    }

    public function Update_Manager_coldcall(Request $request)
    {
        ...
        if (isset($request->status) && $request->status == 1) {
            ...
            if ($request->email != $update_coldcall->email) {
                if (DB::table('meetings')->where('email', $request->email)->exists()) {
        ...
        if ($saveCall) {
            return redirect(route('manager.view-coldcall'))->with('success', 'Cold call update successfully!');
        }
    }

    public function ManagerColdCall_delete($id)
    {
    ...
    public function Archive_Mcold_Call(Request $request)
    {
        if ($request->ajax()) {
            $data = DB::table('meetings')
                ->where('user_id', Session::get('user_id'))
            ...
            return DataTables::of($data)->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $btn = '<a class="btn btn-primary btn-sm m-1" href="' . route('manager.active-coldcall', ['id' => Crypt::encrypt($row->id)]) . '"> <i class="bi bi-arrow-clockwise"></i></a>';
                    return $btn;
                })
                ->rawColumns(['action'])
                ->make(true);
        }
        return view('managerend.archive-mcold-call');
    }

    public function Active_Mcold_Call($id)
    {
        ...
        if ($active) {
            return redirect()->route('manager.archive-coldcall')->with('success', ' Active Cold call successfully!');
        }
    }



    public function ManagerColdCallDetails(Request $request)
    {
        ...
        return view('managerend.manager-coldcall-details', compact('meeting_data', 'temp_service', 'payment'));
    }

    public function AddManagerColdcallService(Request $request)
    {
        ...
        return view('managerend.add-callcold-service', compact('meeting', 'temp_service'));
    }

    public function ManagerColdCallServiceSUBMIT(Request $request)
    {
    ...
    public function ManagerColdCallServiceDiscount(Request $request)
    {
        if (isset($request->meetingid) && $request->meetingid != "") {
            $total = $request->total;
    ...
    public function ManagerColdCallServiceDelete(Request $request)
    {
        ...
        if (isset($request->serviceid) && isset($request->meetingid)) {
            ...
            if ($service_delete) {
                ...
                $meeting_data = DB::table('meetings')->where('id', Crypt::decrypt($request->meetingid))->where('archive', 0)->where('user_type', 3)->where('type', 0)->where('user_id', Session::get('user_id'))->first();
