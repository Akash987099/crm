...
<div class="row">
    ...
    <div class="col-lg-2">
        <nav>
            <ol class="breadcrumb">
                <li><a class="btn btn-danger btn-sm text-white" href="{{route('backend.view-cold-call')}}">
                ...
                <div class="card-body pt-2">
                    <div class="tab-content pt-2">
                        <div class="tab-pane fade show active profile-overview" id="profile-overview">
                            ...
                            @csrf
                            <input type="hidden" id="meetingid" value="@if(isset($meeting->id)){{Crypt::encrypt($meeting->id)}}@endif">
                            <table class="table table-bordered table-scroll mt-3" id="productTable">
                                ...
                                <tfoot>
                                    <form method="post" action="{{route('backend.service-coldcall-discount',['meetingid'=>Crypt::encrypt($meeting->id)])}}">
                                        @csrf
                                        <tr>
                                            <td><b>Total</b></td>
...
<script>
    $(".addProduct").click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{route('backend.service-coldcall-submit')}}",
            data: {
                "_token": "{{ csrf_token() }}",
                "serviceid": serviceid,
                "price": price,
    ...
    $(".remove").click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{route('backend.service-coldcall-delete')}}",
            data: {
                "_token": "{{ csrf_token() }}",
                "serviceid": serviceid,
                "meetingid": meetingid
