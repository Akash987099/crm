                    ...
                    <div class="tab-content pt-2">
                        <div class="tab-pane fade show active profile-overview" id="profile-overview">
                            ...
                            <table class="table table-bordered table-scroll mt-3" id="productTable">
                                ...
                                <tfoot>
                                    <form method="post" action="{{route('bde.coldcall-service-discount',['meetingid'=>Crypt::encrypt($meeting->id)])}}">
                                        @csrf
                                        <tr>
                                            <td><b>Total</b></td>
...
<script>
    $(".addProduct").click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{route('bde.coldcall-service-submit')}}",
            data: {
                "_token": "{{ csrf_token() }}",
                "serviceid": serviceid,
                "price": price,
    ...
    $(".remove").click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{route('bde.coldcall-service-delete')}}",
            data: {
                "_token": "{{ csrf_token() }}",
                "serviceid": serviceid,
                "meetingid": meetingid
