    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p><b>Reschedule Date :</b> <span class="reschedule"></span> </p>
                <p><b>Meeting Assign by:</b> <span class="team_person"></span> </p>
...
<script>
    $(document).on('click', '.client_id', function() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{route('bde.view-client-data')}}",
            data: {
                "_token": "{{ csrf_token() }}",
                "id": dataId
            },
            success: function(response) {
                ...
                $(".custom_price").text(response.data.custom_price);
                $(".followup").text(response.data.followup_date);
                $(".reschedule").text(response.data.reschedule);
                ...
                if (response.data.meeting_status == 1) {
                    ...
                    $(".status").html('<span class="badge rounded-pill bg-info">Follow Up</span>');

                } else if (response.data.meeting_status == 3) {
                    $(".status").html('<span class="badge rounded-pill bg-danger">Reschedule</span>');

                } else {
                    $(".status").html('<span class="badge rounded-pill bg-warning">Padding</span>');
                }
            }
        });
    ...
    $(function() {
        ...
        var table = $('.dataTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('bde.show-assign-meeting') }}",
