...
<div class="pagetitle">
    <div class="row">
        ...
        <div class="col-lg-2">
            <nav>
                <ol class="breadcrumb">
                    <li class="p-1"><a class="btn btn-primary btn-sm text-white" href="{{url('view-clients-list')}}">
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body pb-0 m-4">
                    ...
                    @endif
                    <div class="msg"></div>
                    <form method="post" action="{{route('telemarket.client-details')}}">
                        @csrf
                        <div class="row">
                            <div class="col-lg-6">
                            ...
                            <div class="col-lg-6">
                                <div class="form-group m-2">
                                    ...
                                    @php
                                    $client_status=['1'=>'Heigh','2'=>'Moderate','3'=>'Low'];
                                    @endphp
                                    <select name="client_potential" class="form-control">
                                        ...
                                        <option @if(isset($edit_client->status) && $edit_client->status==$key) {{'selected'}} @endif value="{{$key}}">{{$row}}</option>
                                        @endforeach
                                        @endif
                                    </select>
...
<script>
    ...
    //     $.ajax({
    //         type: 'POST',
    //         url: "{{url('/select-service-getprice')}}",
    //         data: {
    //             "_token": "{{ csrf_token() }}",
    //             "id": getId
    //         },
    ...
    $(document).ready(function() {
        $("#typeofuser").change(function() {
            ...
            $.ajax({
                type: 'POST',
                url: "{{route('telecaller.telemarket.typeofuser')}}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    "id": getId
                },
