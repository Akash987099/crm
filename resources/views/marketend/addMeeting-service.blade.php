...
<div class="row">
    ...
    <div class="col-lg-2">
        <nav>
            <ol class="breadcrumb">
                <li><a class="btn btn-danger btn-sm text-white" href="{{route('market.view-attend-meeting')}}">
                    ...
                    <div class="tab-content pt-2">
                        <div class="tab-pane fade show active profile-overview" id="profile-overview">
                            ...
                            <table class="table table-bordered table-scroll mt-3" id="productTable">
                                ...
                                <tfoot>
                                    <form method="post" action="{{route('market.meeting-service-discount',['meetingid'=>Crypt::encrypt($meeting->id)])}}">
                                        @csrf
                                        <tr>
                                            <td><b>Total</b></td>
...
<script>
    $(".addProduct").click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{route('market.submit-meeting-service')}}",
            data: {
                "_token": "{{ csrf_token() }}",
                "serviceid": serviceid,
                "price": price,
    ...
    $(".remove").click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{route('delete-meeting-service')}}",
            data: {
                "_token": "{{ csrf_token() }}",
                "serviceid": serviceid,
                "meetingid": meetingid
